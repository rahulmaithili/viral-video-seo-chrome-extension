...
export function exportBatchToJSON(jobs: VideoJob[]): void {
  const data = jobs.map((j) => ({
    id: j.id,
    filename: j.filename,
    metadata: j.metadata,
    analysis: j.analysis,
    platformPackage: j.platformPackage,
    validationFeedback: j.validationFeedback,
    status: j.status,
  ...
  link.setAttribute('download', `viral_video_ai_export_${Date.now()}.json`);
